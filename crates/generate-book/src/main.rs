use {
  pulldown_cmark::{CowStr, Event, HeadingLevel, Options, Parser, Tag, TagEnd},
...
impl Chapter<'_> {
  fn title(&self) -> String {
    ...
    self
      ...
      .skip(1)
      .take_while(|event| !matches!(event, Event::End(TagEnd::Heading(..))))
...
fn main() -> Result {
  for language in [Language::English, Language::Chinese] {
    ...
    let txt = fs::read_to_string(format!("docs/guide{}.md", language.suffix()))?;
    ...
    for event in Parser::new_ext(&txt, Options::all()) {
      if let Event::Start(Tag::Heading { level: level @ (HeadingLevel::H2 | HeadingLevel::H3), .. }) = event {
    ...
    for chapter in &mut chapters {
      for event in &mut chapter.events {
        if let Event::Start(Tag::Link { dest_url: dest, .. }) = event {
