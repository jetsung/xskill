...
| `agents_compat` | 否 | `false` | 是否兼容 `.agents/` 资源。为 `true` 时该平台直接读取规范目录，add/remove/restore 跳过 symlink 操作（单平台时输出 `Skipped`，多平台时静默跳过）。find TUI 正常列出和选择，安装时静默跳过 symlink。list `-a` 过滤时显示规范目录中所有 skill（视为已链接）。 |
...
`agents_compat` 为 `true` 的平台直接读取规范目录（`.agents/skills/`），无需创建 symlink 或复制文件。add、remove、restore 均跳过该平台的 symlink 操作。单平台指定（`-a <name>`）时输出 `Skipped: <name> (agents_compat)`（暗灰色）；多平台（`-a '*'`）时静默跳过。find TUI 正常列出和选择，安装时静默跳过 symlink（仍显示在 `Symlinked` 列表中）。list `-a` 过滤时显示规范目录中所有 skill（视为已链接）。
...
* **交互流程**：
  ...
  2. **选择目标平台**：TUI 多选，列表首项为 `Default`（不可选中，因 skim Default 布局倒序显示，视觉上在底部），后续为配置中所有 platforms（含 `agents_compat` 平台）。选中行文字使用蓝色（`Blue`），深色背景高亮。顶部提示 `TAB: select/deselect`，上下各空一行。TAB 选中平台后按 Enter 确认；未选中任何平台直接按 Enter（光标在 `Default` 处），则不创建任何平台符号链接。
